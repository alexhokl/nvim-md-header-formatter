local config = require("nvim-md-header-formatter.config")
	...
	vim.api.nvim_create_autocmd("FileType", {
		...
		callback = function(args)
			for _, level in ipairs(opts.levels) do
				vim.keymap.set("n", opts.keymap_prefix .. level, function()
					-- Stash the level so the operatorfunc knows which level to apply
					-- when re-invoked by `.`.
					M._pending_level = level
					vim.o.operatorfunc = "v:lua.require'nvim-md-header-formatter'.set_header_op"
					return "g@_"
				end, {
					buffer = args.buf,
					expr = true,
